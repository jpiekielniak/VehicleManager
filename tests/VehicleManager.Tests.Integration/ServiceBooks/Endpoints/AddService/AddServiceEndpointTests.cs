...
public class AddServiceEndpointTests : EndpointTests
{
    ...
    public async Task post_add_service_without_authentication_should_return_401_status_code()
    {
        ...
        var response =
            await Client.PostAsJsonAsync(
                ServiceBookEndpoints.Services.Replace("{serviceBookId:guid}", Guid.NewGuid().ToString()), command);
    ...
    [Fact]
    public async Task post_add_service_with_non_existing_service_book_should_return_400_status_code()
    {
        //Arrange
        var command = _factory.CreateAddServiceCommand();
    ...
    [Fact]
    public async Task post_add_service_with_valid_data_should_return_201_status_code()
    {
        //Arrange
        ...
        //Act
        var response = await Client.PostAsJsonAsync(
            ServiceBookEndpoints.Services.Replace("{serviceBookId:guid}", command.ServiceBookId.ToString()), command);
